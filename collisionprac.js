...
const game = {move:{}, x1:box1.offsetLeft, y1:box1.offsetTop , 
x2:box2.offsetLeft, y2: box2.offsetTop, 
speed:5, w1:box1.offsetWidth, h1:box1.offsetHeight, 
w2:box2.offsetWidth, h2:box2.offsetHeight};
...
function mover(){ 
   ...
   box1.style.top = `${game.y1}px`;
    ...
    //box 2//
    if (keyz.KeyD && game.x2 < dim[0]) game.x2 += game.speed;
    if (keyz.KeyA && game.x2 > 0) game.x2 -= game.speed;
    if (keyz.KeyS && game.y2 < dim[1]) game.y2 += game.speed;
    ...
    W:${game.w2} H;${game.h2}`;
    const o3 = game.x1 < (game.x2 + game.w2 )
    && (game.x1 + game.w1) >game.x2
    const o4 = game.y1 < (game.y2 + game.h2 )
    ...
    output3.innerHTML =`H ${o3}: (${game.x1} < ${game.x2} + ${game.x2 + game.w2}) &&
     ((${game.x1} + ${game.w1}) > ${game.x2})`;
    output4.innerHTML =`H ${o3}: (${game.x1} < ${game.x2} + ${game.x2 + game.w2}) &&
     ((${game.x1} + ${game.w1}) > ${game.x2})`;
    output5.innerHTML =`V ${o4}:${game.y1} < (${game.y2} + ${game.h2}) &&
     (${game.y1} + ${game.h1}) > ${game.y2}`;
